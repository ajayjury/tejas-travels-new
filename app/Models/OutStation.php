...
class OutStation extends Model
{
    ...
    public function getAmountArray($distance, $from_date, $to_date=null){
        ...
        $arr['total_km'] = "Total effective Kms: <span style='font-weight:900;color:#000;'>".(floatval($distance)*2)."kms.</span> ";
        ...
        $arr['total_amount'] = "Amount For Effective Kms: <span>Rs.".$this->round_price_per_km." * ".(floatval($distance)*2)."</span> <span style='font-weight:900;color:#000;'>= Rs.".($this->totalAmount($distance))."</span>";
        ...
        $arr['GST'] = "GST (@".$this->gst."%) : <span style='font-weight:900;color:#000;'>Rs.".$this->gstAmount($distance)."</span>";
        $arr['estimated_total_fare'] = "Estimated Total Fare : <span style='font-weight:900;color:#000;'>Rs.".($this->totalAmount($distance)+($this->driver_charges_per_day*$days))+$this->gstAmount($distance)."</span>";
        $arr['Discount'] = "Discount (@".$this->discount."%) : <span style='font-weight:900;color:#000;'>Rs.".$this->discountAmount($distance)."</span>";
        $arr['tejas_price'] = "Tejas Travel's Price : <span style='font-weight:900;color:#000;'>Rs. ".$this->finalAmount($distance)."</span>";
        $arr['advance'] = "Advance Payable (@".$this->advance_during_booking."%) : <span style='font-weight:900;color:#000;'>Rs. ".$this->advanceAmount($distance)."</span>";
        $arr['message'] = "New hour billing starts when usage more than 30 mins.";
        $arr['final_amount'] = $this->finalAmount($distance);
        return $arr;
    }
