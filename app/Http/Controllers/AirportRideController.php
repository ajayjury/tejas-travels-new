...
use App\Models\SpecialFareAirportRide;
use App\Models\AirportRideSubCity;
use Illuminate\Support\Facades\Validator;
use App\Support\For\BookingType;
use App\Models\Common;
use App\Exports\AirportRideExport;
...
class AirportRideController extends Controller
{
    ...
    public function create() {
        ...
        return view('pages.admin.airportride.create')->with('states', State::all())->with('vehicletypes', VehicleType::all())->with('bookingtypes', BookingType::lists());
    }
    ...
    public function store(Request $req) {
        ...
        $messages = array(
            ...
            'airport_id.required' => 'Please enter the airport !',
            'airport_id.regex' => 'Please enter the valid airport !',
        ...
        for($i=0; $i < count($req->start_date); $i++) { 
            $city = new SpecialFareAirportRide;
            $city->airportride_id = $country->id;
        ...
        if($result){
            return response()->json(["url"=>empty($req->refreshUrl)?URL::to('/').'/admin/management/panel/outstation':$req->refreshUrl, "message" => "Data Stored successfully.", "data" => $country], 201);
        }else{
    ...
    public function update(Request $req, $id) {
        ...
        $deleteSpecialFareAirportRide = SpecialFareAirportRide::where('airportride_id',$country->id)->delete();
        ...
        $deleteAirportRideSubCity = AirportRideSubCity::where('airportride_id',$country->id)->delete();
        ...
        if($result){
            return response()->json(["url"=>empty($req->refreshUrl)?URL::to('/').'/admin/management/panel/airport-ride':$req->refreshUrl, "message" => "Data Stored successfully.", "data" => $country], 201);
        }else{
            return response()->json(["error"=>"something went wrong. Please try again"], 400);
        }
    }
    ...
    public function delete($id){
        $country = AirportRide::findOrFail($id);
        $country->delete();
        return redirect()->intended(route('airportride_view'))->with('success_status', 'Data Deleted successfully.');
    }

    public function view(Request $request) {
        if ($request->has('search') || $request->has('state') || $request->has('city') || $request->has('vehicle')) {
            $search = $request->input('search');
            $country = AirportRide::with(['State','City','Vehicle','VehicleType']);
            ...
            $country = AirportRide::orderBy('id', 'DESC')->paginate(10);
        }
        return view('pages.admin.airportride.list')->with('country', $country)->with('bookingtype', BookingType::lists());
    }
    ...
    public function excel(){
        return Excel::download(new AirportRideExport, 'airportride.xlsx');
    }
