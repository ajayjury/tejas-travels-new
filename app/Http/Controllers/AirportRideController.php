...
class AirportRideController extends Controller
{
    ...
    public function update(Request $req, $id) {
        ...
        if($result){
            return response()->json(["url"=>empty($req->refreshUrl)?URL::to('/').'/admin/airport-ride':$req->refreshUrl, "message" => "Data Stored successfully.", "data" => $country], 201);
        }else{
            return response()->json(["error"=>"something went wrong. Please try again"], 400);
        }
    }
    ...
    public function view(Request $request) {
        if ($request->has('search')) {
            ...
            $country = AirportRide::with(['State','City','Vehicle','VehicleType','PackageType'])->where('terms_condition', 'like', '%' . $search . '%')->orWhere('notes', 'like', '%' . $search . '%')->orWhere('description', 'like', '%' . $search . '%')->orWhereHas('State', function($q)  use ($search){
