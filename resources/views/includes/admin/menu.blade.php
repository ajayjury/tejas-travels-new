                            ...
                            <li class="nav-item">
                                ...
                                <div class="collapse menu-dropdown {{strpos(url()->current(),'holiday-package') !== false || strpos(url()->current(),'dynamic-web-pages') !== false || strpos(url()->current(),'faq') !== false || strpos(url()->current(),'seo-meta') !== false || strpos(url()->current(),'list-layout') !== false || strpos(url()->current(),'content-layout') !== false || strpos(url()->current(),'vehicle-type-seo') !== false || strpos(url()->current(),'vehicle-seo') !== false || strpos(url()->current(),'holiday-package-seo') !== false || strpos(url()->current(),'testimonial') !== false || strpos(url()->current(),'gallery') !== false ? 'show' : ''}}" id="sidebarDashboards6">
                                    <ul class="nav nav-sm flex-column">
                                        ...
                                        <li class="nav-item">
                                            <a href="{{route('gallery_view')}}" class="nav-link {{strpos(url()->current(),'gallery') !== false ? 'active' : ''}}" data-key="t-analytics"> Gallery </a>
                                        </li>
                                    </ul>
                                </div>
                            </li> <!-- end Dashboard Menu -->
