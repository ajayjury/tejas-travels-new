...
const choicesAirport = new Choices('#airport', {
      silent: false,
      items: [],
      choices: [
        {
            value: 'Select a airport',
        ...
        @foreach($airports as $airports)
          {
              value: '{{$airports->id}}',
              label: '{{$airports->name}}',
              selected: {{($country->airport->id==$airports->id) ? 'true' : 'false'}},
          },
        @endforeach
      ],
      renderChoiceLimit: -1,
    ...
    async function(event) {
      ...
      try {
          ...
          if(response.data.airports.length>0){
            choicesAirport.clearStore();
            choicesAirport.clearChoices();
            response.data.airports.forEach((item)=>{
                data.push({value: item.id, label: item.name,})
            })
            choicesAirport.setChoices(data);
          }else{
            choicesAirport.clearStore();
            choicesAirport.clearChoices();
          }
