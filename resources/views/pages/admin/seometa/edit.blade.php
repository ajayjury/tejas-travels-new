...
<div class="page-content">
    <div class="container-fluid">
        ...
        <!-- end page title -->
        <form id="countryForm" method="post" action="{{route('seometa_update', $country->id)}}" enctype="multipart/form-data">
            @csrf
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header align-items-center d-flex">
                        <h4 class="card-title mb-0 flex-grow-1">SEO META</h4>
                    </div><!-- end card header -->
                    <div class="card-body">
                            ...
                            <div class="row gy-4">
                                <div class="col-xxl-6 col-md-12">
                                    <div>
                                        ...
                                        <textarea class="form-control" name="header" id="header" cols="30" rows="10">{!!$country->header!!}</textarea>
                                        @error('header') 
                                            <div class="invalid-message">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-xxl-6 col-md-12">
                                    <div>
                                        ...
                                        <textarea class="form-control" name="footer" id="footer" cols="30" rows="10">{!!$country->footer!!}</textarea>
                                        @error('footer') 
                                            <div class="invalid-message">{{ $message }}</div>
                                        @enderror
                                    </div>
  ...
  .onSuccess(async (event) => {
    // event.target.submit();
      ...
      try {
        ...
        const response = await axios.post('{{route('seometa_update', $country->id)}}', formData)
        ...
        if(error?.response?.data?.form_error?.header){
            errorToast(error?.response?.data?.form_error?.header[0])
        }
        if(error?.response?.data?.form_error?.footer){
            errorToast(error?.response?.data?.form_error?.footer[0])
        }
      } finally{
            submitBtn.innerHTML =  `
                Update
