                                ...
                                <div class="col-xxl-12 col-md-12">
                                    <div>
                                        ...
                                        @foreach($access_list as $key=>$value)
                                            <label for="access_{{$key}}" style="margin-right:10px"><input type="checkbox" id="access_{{$key}}" class="" value="{{$key}}" name="access[]" @if(in_array($key, json_decode($country->access))) checked @endif /> {{$value}}</label><br/>
                                        @endforeach
                                        @error('access') 
                                            <div class="invalid-message">{{ $message }}</div>
                                        @enderror
  ...
  .onSuccess(async (event) => {
    // event.target.submit();
      ...
      try {
        ...
        const response = await axios.post('{{route('access_level_update', $country->id)}}', formData)
