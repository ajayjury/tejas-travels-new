...
const choicesCities = new Choices('#city', {
    silent: false,
    items: [],
    choices: [
            {
            ...
            @foreach($cities as $cities)
            {
                value: '{{$cities->id}}',
                label: '{{$cities->name}}',
                selected: {{$state->city_id == $cities->id ? 'true' : 'false'}},
            },
            @endforeach
  ...
  function setCityData(data){
      if(data.state_id == document.getElementById('state').value){
  ...
  async function(event) {
    ...
    try {
        ...
        if(response.data.cities.length>0){
            ...
            choicesCities.setChoices(data);
        }else{
            choicesCities.clearChoices();
            choicesCities.setChoices([
                {value: 'Select a city', label: 'Select a city', disabled: true, selected: true}
            ]);
        }
    } catch (error) {
        console.log(error);
